...
describe('ToggleFavoriteUseCase', () => {
  ...
  describe('validation errors', () => {
    it('should throw error when movie ID is empty', async () => {
      await expect(toggleFavoriteUseCase.execute('')).rejects.toThrow(
    ...
    it('should throw error when movie ID is null', async () => {
      await expect(toggleFavoriteUseCase.execute(null as any)).rejects.toThrow(
        'Movie ID is required'
      );
    });
    ...
    it('should throw error when movie ID is undefined', async () => {
      await expect(
        toggleFavoriteUseCase.execute(undefined as any)
  ...
  describe('error handling', () => {
    it('should throw error when movie not found', async () => {
      ...
      await expect(
        toggleFavoriteUseCase.execute('nonexistent')
    ...
    it('should throw error when repository findById fails', async () => {
      ...
      await expect(toggleFavoriteUseCase.execute('1')).rejects.toThrow(
        'Database error'
      );
    ...
    it('should throw error when repository update fails', async () => {
      ...
      await expect(toggleFavoriteUseCase.execute('1')).rejects.toThrow(
        'Failed to toggle favorite for movie with ID: 1'
      );
