...
describe('UpdateMovieUseCase', () => {
  ...
  describe('validation errors', () => {
    ...
    it('should throw error for invalid title', async () => {
      await expect(
        updateMovieUseCase.execute('1', { title: '' })
      ).rejects.toThrow();
    });

    it('should throw error for invalid director', async () => {
      await expect(
        updateMovieUseCase.execute('1', { director: '' })
      ).rejects.toThrow();
    });

    it('should throw error for invalid genre', async () => {
      await expect(
        updateMovieUseCase.execute('1', { genre: '' })
      ).rejects.toThrow();
    });

    it('should throw error for invalid release year', async () => {
      await expect(
        updateMovieUseCase.execute('1', { releaseYear: 1799 })
      ).rejects.toThrow();
    });

    it('should throw error for invalid rating', async () => {
      await expect(
        updateMovieUseCase.execute('1', { rating: -1 })
      ).rejects.toThrow();

      await expect(
        updateMovieUseCase.execute('1', { rating: 11 })
      ).rejects.toThrow();
    });
  });
  ...
  describe('error handling', () => {
    it('should throw error when movie ID is empty', async () => {
      await expect(
        updateMovieUseCase.execute('', { title: 'New Title' })
    ...
    it('should throw error when movie not found', async () => {
      ...
      await expect(
        updateMovieUseCase.execute('nonexistent', { title: 'New Title' })
    ...
    it('should throw error when repository update fails', async () => {
      ...
      await expect(
        updateMovieUseCase.execute('1', { title: 'New Title' })
